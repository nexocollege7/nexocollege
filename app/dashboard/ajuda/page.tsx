...
export default function AjudaPage() {
  ...
  async function createTicket() {
    ...
    const { data: ticket, error } = await supabase
      .from('support_tickets')
      .insert({ school_id: schoolId, title: form.title, category: form.category, ticket_type: 'aluno_escola' })
      .select()
      .single()
    if (error || !ticket) { setSending(false); return }
    await supabase.from('support_messages').insert({
      ticket_id: ticket.id, sender_id: userId, sender_role: 'student', content: form.firstMessage,
    })
    setForm({ title: '', category: 'duvida_conteudo', firstMessage: '' })
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#fff', margin: 0 }}>Ajuda</h1>
          <p style={{ color: '#888', margin: '4px 0 0', fontSize: '14px' }}>Tire suas dúvidas diretamente com a escola</p>
        </div>
        ...
        <div>
          {tickets.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#555', background: '#111', borderRadius: '12px', border: '1px dashed #333' }}>
              ...
              <p style={{ margin: 0 }}>Nenhuma solicitação ainda.</p>
              <p style={{ margin: '8px 0 0', fontSize: '13px' }}>Clique em "Pedir Ajuda" para falar com a escola.</p>
            </div>
          ) : (
              ...
              {messages.map(msg => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', justifyContent: isStudent ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '75%', background: isStudent ? '#7C4DFF' : '#1e1e1e', color: '#fff', borderRadius: isStudent ? '12px 12px 2px 12px' : '12px 12px 12px 2px', padding: '10px 14px', fontSize: '14px' }}>
                      {!isStudent && <p style={{ margin: '0 0 4px', fontSize: '11px', color: '#AEEA00', fontWeight: '600' }}>Escola</p>}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#111', border: '1px solid #333', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '480px', margin: '0 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <h2 style={{ margin: 0, color: '#fff', fontSize: '18px' }}>Pedir Ajuda</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ color: '#aaa', fontSize: '13px', display: 'block', marginBottom: '6px' }}>Título</label>
                <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Ex: Não consigo acessar a aula" style={{ width: '100%', background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', padding: '10px 14px', color: '#fff', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
              ...
              <div>
                <label style={{ color: '#aaa', fontSize: '13px', display: 'block', marginBottom: '6px' }}>Como podemos ajudar?</label>
                <textarea value={form.firstMessage} onChange={e => setForm({...form, firstMessage: e.target.value})} placeholder="Descreva sua dúvida com detalhes..." rows={4} style={{ width: '100%', background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', padding: '10px 14px', color: '#fff', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              <button onClick={createTicket} disabled={sending || !form.title.trim() || !form.firstMessage.trim()} style={{ background: '#AEEA00', color: '#000', border: 'none', borderRadius: '8px', padding: '12px', fontWeight: '700', fontSize: '15px', cursor: 'pointer', opacity: sending ? 0.6 : 1 }}>
                {sending ? 'Enviando...' : 'Enviar Pedido de Ajuda'}
              </button>
            </div>
          </div>
        </div>
