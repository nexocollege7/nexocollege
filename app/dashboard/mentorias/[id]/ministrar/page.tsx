...
import { getMentorship, getCohorts, updateClassScheduledAt } from '@/app/actions/mentor-actions'
import VideoRoom from '@/components/ui/VideoRoom'

export default function MinistrarMentoriaPage() {
  ...
  useEffect(() => {
    async function load() {
      const [mentoriaData, cohortsData] = await Promise.all([
        getMentorship(id),
        getCohorts(id),
      ])
      ...
      <div>
        ...
        <p style={{ color: '#888', fontSize: '14px', margin: 0 }}>
          {turmaAberta
            ...
            : 'Nenhuma turma aberta — crie uma turma antes de ministrar'}
        </p>
      </div>
                  ...
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap', marginTop: '6px' }}>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await updateClassScheduledAt(c.id, id, iso)
